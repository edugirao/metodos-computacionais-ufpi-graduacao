PROGRAM oblique_launch
IMPLICIT NONE
INTEGER:: i,nt
REAL(KIND=8):: m,g,b,dt,x0,y0,vx0,vy0,v0,alpha,pi
...
DO i=1,nt-1
  ...
  k1vx=dt*fvx(t(i),x(i),y(i),vx(i),vy(i),b,m)
  k1vy=dt*fvy(t(i),x(i),y(i),vx(i),vy(i),b,m,g)
  ...
  k2vx=dt*fvx(t(i)+0.5D0*dt,x(i)+0.5D0*k1x,y(i)+0.5D0*k1y,vx(i)+0.5D0*k1vx,vy(i)+0.5D0*k1vy,b,m)
  k2vy=dt*fvy(t(i)+0.5D0*dt,x(i)+0.5D0*k1x,y(i)+0.5D0*k1y,vx(i)+0.5D0*k1vx,vy(i)+0.5D0*k1vy,b,m,g)
  ...
  k3vx=dt*fvx(t(i)+0.5D0*dt,x(i)+0.5D0*k2x,y(i)+0.5D0*k2y,vx(i)+0.5D0*k2vx,vy(i)+0.5D0*k2vy,b,m)
  k3vy=dt*fvy(t(i)+0.5D0*dt,x(i)+0.5D0*k2x,y(i)+0.5D0*k2y,vx(i)+0.5D0*k2vx,vy(i)+0.5D0*k2vy,b,m,g)
  
  k4x=dt*fx(t(i)+dt,x(i)+k3x,y(i)+k3y,vx(i)+k3vx,vy(i)+k3vy)
  k4y=dt*fy(t(i)+dt,x(i)+k3x,y(i)+k3y,vx(i)+k3vx,vy(i)+k3vy)
  k4vx=dt*fvx(t(i)+dt,x(i)+k3x,y(i)+k3y,vx(i)+k3vx,vy(i)+k3vy,b,m)
  k4vy=dt*fvy(t(i)+dt,x(i)+k3x,y(i)+k3y,vx(i)+k3vx,vy(i)+k3vy,b,m,g)
...
END PROGRAM oblique_launch

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

FUNCTION fvx(t,x,y,vx,vy,b,m)
IMPLICIT NONE
REAL(KIND=8):: fvx,t,x,y,vx,vy,b,m
fvx=-b*vx/m
END FUNCTION fvx

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

FUNCTION fvy(t,x,y,vx,vy,b,m,g)
IMPLICIT NONE
REAL(KIND=8):: fvy,t,x,y,vx,vy,b,m,g
fvy=-g-b*vy/m
END FUNCTION fvy
